...
app.use(express.urlencoded({extended:false})).use(express.json()).use(cors()).use( jwt({ secret: privateKey, algorithms: ["HS256"] }).unless({path:['/user/login','/user/reguser','/user/getCode','/user/retrievePassword']})).use(resData).use(db).use('/user',userRouter)
